...
def get_stock(symbol):
    try:
        ...
        # Helper: fetch Yahoo quoteSummary for richer metadata (industry, exchange, market cap, description, website)
        def fetch_quote_summary(sym: str):
            try:
                url = f'https://query1.finance.yahoo.com/v10/finance/quoteSummary/{sym}'
                params = { 'modules': 'price,assetProfile,summaryProfile,summaryDetail' }
                headers = {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36',
                    'Accept': 'application/json, text/plain, */*',
                    'Referer': 'https://finance.yahoo.com/'
        ...
        logger.info(f"QuoteSummary response for {symbol}: {qs}")
        try:
        ...
        try:
            ...
            pass
        # Enrich exchange and market cap from quoteSummary.price when available
        try:
            price_mod = (qs.get('price') or {})
            if not derived_exchange:
                derived_exchange = price_mod.get('fullExchangeName') or price_mod.get('exchangeName') or derived_exchange
        except Exception:
            pass
        ...
        # Derive industry/description/logo from quoteSummary.assetProfile/summaryProfile
        try:
            asset_prof = (qs.get('assetProfile') or {})
            summ_prof = (qs.get('summaryProfile') or {})
        except Exception:
            asset_prof, summ_prof = {}, {}
        try:
            description_val = asset_prof.get('longBusinessSummary') or summ_prof.get('longBusinessSummary')
        except Exception:
            description_val = None
        # Best-effort favicon from website as logo
        try:
            website = asset_prof.get('website') or ''
            logo_url = f"https://www.google.com/s2/favicons?domain={website}&sz=64" if website else ''
        except Exception:
            logo_url = ''

        # Ensure we have some reasonable values even if APIs fail
        if not derived_exchange or derived_exchange == 'N/A':
            derived_exchange = 'Unknown Exchange'
        ...
        # Optionally include description for UI if needed
        if description_val:
            response['description'] = description_val
